    ...
    await worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      ...
      if (values[2] && values[2] !== currentPage) {
        if (sheetName in typedSheetList) {
          ...
          console.error(`Sheet '${sheetName}' not found in SHEET_LIST.`);
        }
...
const sanitizeCellValue = (cellValue: string | CellValueObject | undefined): string => {
  ...
  if (typeof cellValue === "string") {
    ...
    if (cellValue.richText) {
      // Rich Text Type
      transformedValue = cellValue.richText.map((rt: RichText) => rt.text || "").join("");
