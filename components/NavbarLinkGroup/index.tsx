...
export function LinksGroup({ icon: Icon, image, label, link, links, arrowMenu }: LinksGroupProps) {
	...
	const items = (hasLinks ? links : []).map((link) => (
		...
		>
			<div className="border-l-2 pl-5 border-[#74c0fc]">
				{link.label}
			</div>
		</Link>
	));
	...
	return (
		<>
			...
			onClick={() => {
				if(!link){
					if (!Array.isArray(dropDownSideBar)) {
						...
						dispatch({
							type: GENERAL.SET_DROPDOWN_SIDEBAR_STATE,
							payload: dropDownSideBar.filter((f) => f !== label)
						});
					}
				}
			}}
			className={`cursor-pointer ${classes.control}`}>
				...
				{hasLinks && (
					...
					style={{
						width: rem(16),
						height: rem(16),
						transform: Array.isArray(dropDownSideBar) && dropDownSideBar.includes(label) ? 'rotate(-90deg)' : 'none',
					}}
					/>
				)}
				{arrowMenu && (
			...
			{hasLinks ? <Collapse in={Array.isArray(dropDownSideBar) && dropDownSideBar.includes(label)}>{items}</Collapse> : null}
		</>
	);
}
